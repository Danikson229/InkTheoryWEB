        ...
        ">
            ...
            <div style="flex-shrink: 0;">
                ...
                <p style="
                    ...
                    font-size: 13px;
                ">
                    ${d.lblSize}: ${escapeHTML(selection.size)}
                    |
                    ${d.lblColor}: ${escapeHTML(colorText)}
                    |
                    ${d.lblFit}: ${escapeHTML(fitText)}
                </p>
            ...
            >
                ...
                <!-- HONEYPOT: скрытое поле-ловушка от FormSubmit.
                     Настоящие пользователи его не видят и не заполняют,
                     а простые боты часто заполняют все поля подряд —
                     тогда FormSubmit тихо отбрасывает заявку. -->
                <input
                    type="text"
                    name="_honey"
                    style="display:none"
                ...
                <input
                    type="hidden"
                    name="product"
                    value="${escapeHTML(product.name)} (${escapeHTML(selection.size)}/${escapeHTML(selection.color)}/${escapeHTML(fitText)})"
                >
                ...
                onApprove:
                    async function(data, actions) {
                        ...
                        try {
                             ...
                             *
                             * ВАЖНО (см. рекомендации по безопасности):
                             * эта проверка выполняется в браузере клиента,
                             * а значит принципиально не является надёжной
                             * защитой от подделки. Для полноценной защиты
                             * от "бесплатных" заказов необходима серверная
                             * проверка заказа через PayPal API/Webhooks
                             * перед тем, как считать оплату подтверждённой.
                             */
